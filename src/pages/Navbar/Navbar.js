  ...
  <div className="container flex flex-wrap justify-between  items-center mx-auto">
        ...
        <div className="relative">
          <div className={(profileDropdown ? "profile-dropdown-active shadow-md bg-white text-sm rounded-lg mt-2" : "profile-dropdown-inactive")}>
            <div className="flex flex-col" ref={ref}>
                <ViewUser to={`../viewuser/${userId}`} onClick={(event)=>{activateProfileDropDown();}} className="cursor-pointer hover:bg-gray-200 rounded-t-lg p-2">
                  <i className="fas fa-user mx-2 text-gray-600"></i>
                  Perfil</ViewUser>
                <ViewUser to={`../reservedproducts/${userId}`} onClick={(event)=>{activateProfileDropDown();}} className="cursor-pointer hover:bg-gray-200 p-2">
    ...
    <div className={`justify-between py-2.5 items-center  mx-6 w-full md:flex md:w-auto md:order-1 ${show}`}  id="mobile-menu-3">
      ...
      <ul className="flex flex-col mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium">
        ...
        {!isLogged &&
        <li>
          <LoginRegister to={`../signinup/${1}`} className="block m-2 cursor-pointer pr-4 pl-3 text-white  hover:bg-gray-50 hover:bg-transparent hover:text-gray-400 md:p-0 ">Iniciar Sesión</LoginRegister>
        </li>}
        {!isLogged &&
        <li>
          <LoginRegister to={`../signinup/${2}`} className="block m-2 cursor-pointer pr-4 pl-3 text-white hover:bg-gray-50 hover:bg-transparent hover:text-gray-400 md:p-0 ">Registrarse</LoginRegister>
        </li>}
        {isLogged &&
        <li>
        ...
        <>
        <li>
          <ViewUser to={`../viewuser/${userId}`} className="block m-2 md:hidden cursor-pointer pr-4 pl-3 text-white hover:bg-gray-50 hover:bg-transparent hover:text-gray-400 md:p-0 ">
            Perfil</ViewUser>
        </li>
        <li>
          <ViewUser to={`../reservedproducts/${userId}`} className="block m-2 md:hidden cursor-pointer pr-4 pl-3 text-white hover:bg-gray-50 hover:bg-transparent hover:text-gray-400 md:p-0 ">
            Reservas</ViewUser>
        </li>
        <li>
