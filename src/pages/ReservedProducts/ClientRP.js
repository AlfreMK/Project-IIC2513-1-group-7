...
const ClientRP = (props) => {
  ...
  const deleteReserve = (id) => {
	...
	const cancelAll = () => {
		axios.put(
			`${process.env.REACT_APP_SERVER_URL}/rproducts/cancelAll/${1}/${userId}`)
			.then(res => {
				if (res.status === 200){
	...
	const deleteAll = () => {
		axios.delete(
			`${process.env.REACT_APP_SERVER_URL}/rproducts/deleteAll/${1}/${userId}`)
			.then(res => {
				if (res.status === 200){
